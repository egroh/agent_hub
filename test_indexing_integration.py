...
sys.path.append("backend")  # noqa: E402

from backend.app.services.indexer.service import IndexerService  # noqa: E402
from backend.app.db.github_data_service import GithubDataService  # noqa: E402
from backend.app.database import get_async_session  # noqa: E402
from backend.app.models.models import Repository, RepositoryFile, CodeUnit  # noqa: E402
...
async def test_indexing_and_db_integration():
    ...
    try:
        ...
        async for session in get_async_session():
            ...
            try:
                status = await github_data_service.get_repository_status(
                    owner, repo, session
                )
            ...
            if repository:
                ...
                print(
                    f'  Description: {repository.description[:100] if repository.description else "None"}...'
                )
            else:
            ...
            if files:
                for i, file in enumerate(files[:5]):  # Show first 5 files
                    print(
                        f'  File {i+1}: {file.path} ({file.language}) - {file.description[:50] if file.description else "No description"}...'
                    )
                if len(files) > 5:
            ...
            if code_units:
                ...
                if functions:
                    print(
                        f'  Example function: {functions[0].name} - {functions[0].description[:50] if functions[0].description else "No description"}...'
                    )
                if classes:
                    print(
                        f'  Example class: {classes[0].name} - {classes[0].description[:50] if classes[0].description else "No description"}...'
                    )
            ...
            final_status = await github_data_service.get_repository_status(
                owner, repo, session
            )
