...
class ChatService:
    ...
    async def chat_with_repository(
        self, owner: str, repo: str, chat_request: ChatRequest, session: AsyncSession
    ) -> ChatResponse:
        """
        Mock implementation of chat with repository functionality
        """
        self.logger.info(
            f"Processing chat request for {owner}/{repo}: {chat_request.message}"
        )
        ...
        if "hello" in message or "hi" in message:
            ...
            # Mock source files
            source_files = [{"README.md": "docs/README.md"}, {"main.py": "src/main.py"}]
            ...
            # Mock code snippets
            code_snippets = [{"function": "main", "file": "src/main.py", "line": 10}]
            source_files = [{"main.py": "src/main.py"}]
        else:
