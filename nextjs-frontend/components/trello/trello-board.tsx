...
export function TrelloBoard() {
    ...
    const fetchInitialState = async () => {
      try {
        ...
        if (
          ...
          response.data.card_data.length > 0
        ) {
          console.log("Loaded initial demo data:", response.data.card_data);

          const newCards: TaskCard[] = response.data.card_data.map(
            (card: any) => ({
              id: card.card_id,
              title: card.title,
          ...
          setColumns((prevColumns) => {
            ...
            newCards.forEach((card) => {
              const columnIndex = newColumns.findIndex(
                (col) => col.id === card.containerId,
              );
              if (columnIndex !== -1) {
                // Check if card already exists to avoid duplicates
                if (
                  !newColumns[columnIndex].cards.some((c) => c.id === card.id)
                ) {
  ...
  const moveCardToColumn = useCallback(
    (cardId: string, targetColumn: "todo" | "doing" | "done") => {
      setColumns((prevColumns) => {
        const newColumns = [...prevColumns];
  ...
  const startTaskExecution = useCallback(
    (cardId: string, executionType: TaskExecution["executionType"]) => {
      // Start execution tracking
      const executionId = taskExecutionService.startExecution(
        cardId,
        executionType,
      );
      ...
      setColumns((prevColumns) => {
        ...
        const doingColumnIndex = newColumns.findIndex(
          (col) => col.id === "doing",
        );
  ...
  const checkParentTaskCompletion = useCallback((parentTaskId: string) => {
    setColumns((prevColumns) => {
      ...
      for (let colIndex = 0; colIndex < newColumns.length; colIndex++) {
        const cardIndex = newColumns[colIndex].cards.findIndex(
          (card) => card.id === parentTaskId,
        );
        if (cardIndex !== -1) {
      ...
      if (
        !parentCard ||
        !parentCard.subTaskIds ||
        parentCard.subTaskIds.length === 0
      ) {
        return prevColumns;
      }
      ...
      if (completedSubTasks === totalSubTasks && parentCard.status !== "done") {
        ...
        // Add to Done column
        const doneColumnIndex = newColumns.findIndex(
          (col) => col.id === "done",
        );
        if (doneColumnIndex !== -1) {
          ...
          console.log(
            `🎉 Parent task "${parentCard.title}" completed - all sub-tasks done!`,
          );
        }
      } else {
        // Update parent task in current column
        newColumns[parentColumnIndex].cards[parentCardIndex] =
          updatedParentCard;
      }
  ...
  const sortTasksByDependencyOrder = useCallback(
  ...
  const executeSubTasksSequentially = useCallback(
    async (subTaskIds: string[]) => {
      console.log(
        `🚀 Starting sequential execution of ${subTaskIds.length} sub-tasks`,
      );
      ...
      for (let i = 0; i < sortedTaskIds.length; i++) {
        ...
        try {
          ...
          await new Promise<void>((resolve) => {
            setColumns((prevColumns) => {
              for (const column of prevColumns) {
                const foundTask = column.cards.find(
                  (card) => card.id === subTaskId,
                );
                if (foundTask) {
                  subTask = foundTask;
                  break;
          ...
          console.log(
            `⚡ Executing sub-task ${i + 1}/${sortedTaskIds.length}: "${subTask.title}" (Type: ${subTask.aiMetadata?.taskType || "unknown"})`,
          );
          ...
          if (taskType === "research_task") {
            ...
            console.log(
              `✅ Deep search completed for "${subTask.title}":`,
              apiResponse,
            );
            ...
            console.log(
              `📞 Making outbound call #${callCount + 1} to ${contactName} (${targetNumber}) for: "${taskTitle}"`,
            );
            ...
            apiResponse = response.data;
            console.log(
              `✅ Outbound call completed for "${subTask.title}":`,
              apiResponse,
            );
            ...
            apiResponse = response.data;
            console.log(
              `✅ AI processing completed for "${subTask.title}":`,
              apiResponse,
            );
          }
          ...
          setColumns((prevColumns) => {
            ...
            if (doneColumnIndex !== -1) {
              const taskIndex = newColumns[doneColumnIndex].cards.findIndex(
                (card) => card.id === subTaskId,
              );
              if (taskIndex !== -1) {
                const currentCard =
          ...
          setColumns((prevColumns) => {
            ...
            for (let colIndex = 0; colIndex < newColumns.length; colIndex++) {
              const taskIndex = newColumns[colIndex].cards.findIndex(
                (card) => card.id === subTaskId,
              );
              if (taskIndex !== -1) {
      ...
      console.log(
        `🎉 Completed sequential execution of all ${sortedTaskIds.length} sub-tasks`,
      );
    },
    [
      columns,
      ...
      setColumns((prevColumns) => {
        ...
        if (targetContainer === "done") {
          ...
          if (todoColumnIndex !== -1) {
            ...
            const dependentCards = newColumns[todoColumnIndex].cards.filter(
              (card) =>
                card.dependsOn?.includes(activeCardId) ||
                card.blockedBy?.includes(activeCardId),
            );

            // Remove dependencies and move dependent cards to TODO if they have no other blockers
            dependentCards.forEach((dependentCard) => {
              const cardIndex = newColumns[todoColumnIndex].cards.findIndex(
                (c) => c.id === dependentCard.id,
              );
              if (cardIndex !== -1) {
                const updatedDependentCard = {
                  ...dependentCard,
                  dependsOn:
                    dependentCard.dependsOn?.filter(
                      (id) => id !== activeCardId,
                    ) || [],
                  blockedBy:
                    dependentCard.blockedBy?.filter(
                      (id) => id !== activeCardId,
                    ) || [],
                };
                ...
                if (
                  updatedDependentCard.dependsOn.length === 0 &&
                  updatedDependentCard.blockedBy.length === 0
                ) {
                  // You could add visual indication here that the card is now ready
                  console.log(
                    `Card "${dependentCard.title}" is now ready to work on!`,
                  );
                }

                newColumns[todoColumnIndex].cards[cardIndex] =
                  updatedDependentCard;
              }
            });
          }
          ...
          if (activeCard.isSubTask && activeCard.parentTaskId) {
            ...
            setTimeout(
              () => checkParentTaskCompletion(activeCard.parentTaskId!),
              0,
            );
          }
        }
    ...
    async (updatedCard: TaskCard) => {
      ...
      setColumns((prevColumns) => {
        ...
        if (!existingCardFound) {
          // Add new card
          const targetColumnIndex = newColumns.findIndex(
            (col) => col.id === updatedCard.containerId,
          );
          if (targetColumnIndex !== -1) {
      ...
      if (isNewCard && updatedCard.title.trim()) {
        ...
        setColumns((prevColumns) => {
          ...
          if (doingColumnIndex !== -1) {
            const cardIndex = newColumns[doingColumnIndex].cards.findIndex(
              (card) => card.id === updatedCard.id,
            );
            if (cardIndex !== -1) {
        ...
        try {
          ...
          setColumns((prevColumns) => {
            ...
            if (
              ...
              doingColumnIndex !== -1 &&
              response.card_data &&
              Array.isArray(response.card_data)
            ) {
              // Create sub-tasks from AI response
              const subTaskIds: string[] = [];
              const newCards: TaskCard[] = response.card_data.map(
              ...
              const parentCardIndex = newColumns[
                doingColumnIndex
              ].cards.findIndex((card) => card.id === updatedCard.id);
        ...
        >
          <div className="flex gap-6 overflow-x-auto pb-4">
            ...
            <div className="flex-shrink-0">
              <Button className="w-80 h-12 bg-white/10 hover:bg-white/20 text-white border border-white/20 justify-start">
                <Plus className="w-4 h-4 mr-2" />
                Add another list
              </Button>
            </div>
